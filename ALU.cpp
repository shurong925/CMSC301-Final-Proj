...
string ALU::adder(int one, int two){
    result = bitset<32>(one + two).to_string();
    return result;
}
...
string ALU::getResult()
{
  //string op;
  if(op == "0010"){
    result = bitset<32>((stoll(readDataOne, 0, 2) + stoll(readDataTwo, 0, 2))).to_string();
    return result;
  }
  else if(op == "0110"){
    result = bitset<32>((stoll(readDataOne, 0, 2) - stoll(readDataTwo, 0, 2))).to_string();
    ...
    if(stoll(readDataOne, 0, 2) > stoll(readDataTwo, 0, 2))
          result = readDataTwo;
     else
          result = readDataOne;
     return result;
