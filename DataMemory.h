...
{
 private:
   map<string, string> Memory; //Key is address, value is what is stored at that address
   string address; //The address to read from, from ALU
   string writeData; //The data to be written to memory, from register file
   bool memWrite; //Value from control that indicates if the instruction is a write
    bool memRead; //Value from control that indicates if the instruction is a read
